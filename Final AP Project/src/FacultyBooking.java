...
/**
 * To cancel a booking made by the faculty and the admin
 * 
 * @author Yajur
 * @author Shravika
 *
...
public class FacultyBooking extends Application {
	...
	public void start(Stage primaryStage) throws ClassNotFoundException, IOException {
		...
		/**
		 * Displaying bookings made by the faculty
		 */
		if (type == 1) {
		...
		/**
		 * Displaying bookings made by the admin (for students bookings as well)
		 */
		else if (type == 2) {
		...
		/**
		 * Cancelling the bookings - which also frees the booked time slots
		 */
					...
					if (check_Box_List.get(i).isSelected()) {
						...
						for (int i1 = 0; i1 < App.actual_Room_List.size(); i1++) {
							if (App.actual_Room_List.get(i1).get_Name().equals(room)) {
								App.actual_Room_List.get(i1).getList_Of_Weeks()
										.get(current_User.bookings.get(index - 1).week - 31).getWeek_List()
										.get(current_User.bookings.get(index - 1).day).getday_List()
										.set(h1.indexOf(current_User.bookings.get(index - 1).time), false);
								break;
							}
						}
